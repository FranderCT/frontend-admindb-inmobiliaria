...
export default function ContenidoDetalleContrato({ contractID }: ContractDetailProps) {
  const { contract: detail, loadingContract, fetchingContract, errorContract } = useGetContract(contractID);
  ...
  return (
    <div className="max-h-[85vh] overflow-y-auto pr-4
      ...
      <div className="pt-5 print:pt-0 print:px-0">
        ...
        <section className="mb-6">
          ...
          {detail.tipoContrato === 'Alquiler'  && (
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex justify-between w-1/2 pl-4">
        ...
        <section className="mb-8">
          <h3 className="text-[11px] tracking-widest font-semibold uppercase flex items-center gap-2">
            <FileText className="h-4 w-4" /> Términos y condiciones ({Array.isArray(detail.condiciones) ? detail.condiciones.length : 0})
          </h3>
          <ol className="mt-3 list-decimal pl-5 space-y-2 text-sm leading-6">
            {Array.isArray(detail.condiciones) && detail.condiciones.length > 0 ? (
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              detail.condiciones.map((c: any, idx: number) => {
                const key = c?.idCondicion ?? idx;
                const text = typeof c === 'string' ? c : (c?.textoCondicion ?? String(c));
